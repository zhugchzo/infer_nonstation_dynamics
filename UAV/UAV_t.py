...
df_f_elements.to_csv('../results/UAV/UAV_W_out_t.csv',header = True)
...
dic_pred = {'Time':t_series[train_length + 1:,0],'traj_x':x_tseries[train_length + 1:,0],'traj_y':x_tseries[train_length + 1:,1],'pred_x':predict_traj_x, 'pred_y':predict_traj_y}

csv_out = pd.DataFrame(dic_pred)
csv_out.to_csv('../results/UAV/UAV_pred_t.csv',header = True)
...
dic_gen = {'Time':t_series[1:,0],'traj_x':x_tseries[1:,0],'traj_y':x_tseries[1:,1],'gen_x':gen_traj_x, 'gen_y':gen_traj_y}

csv_out = pd.DataFrame(dic_gen)
csv_out.to_csv('../results/UAV/UAV_gen_t.csv',header = True)