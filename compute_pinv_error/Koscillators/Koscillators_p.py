...
#####################################################################################
    ...
    x_gen_sparse[d:d+d_theta,:] = np.tile(xp_tseries[:train_length, -1], N)
    ...
    for node in range(N):
        ...
        x_gen_sparse[0,train_length*node] = xp_tseries[0,node]
        x_gen_sparse[d+d_theta:,train_length*node] = np.sum(np.sin(xp_tseries[0,inter_var_index]-np.tile(xp_tseries[0,node],k)))
...
print('forcing parameters MSE: {}'.format(mean_MSE))
print('forcing parameters pinv error: {}'.format(mean_pinv_error))