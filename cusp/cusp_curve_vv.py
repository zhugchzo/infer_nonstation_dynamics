...
if not os.path.exists('../results/cusp'):
    os.makedirs('../results/cusp')
...
df_tseries = pd.read_csv('cusp_curve_data.csv')
...
for initial_theta in grid_initial_theta:
    ...
    for delta_theta in grid_delta_theta:
        ...
        lambda_param = 5e-2  # Adjustable parameter, modify according to your data
...
AIC_out.to_csv('../results/cusp/cusp_curve_AIC.csv', index_label='')
...
dic_W_out.to_csv('../results/cusp/cusp_curve_W_out.csv',header=True,index=False)
...
pred_out.to_csv('../results/cusp/cusp_curve_pred.csv',header = True, index=False)