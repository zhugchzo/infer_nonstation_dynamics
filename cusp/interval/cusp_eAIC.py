...
for dt in [0.02,0.03,0.04,0.05]:

    if not os.path.exists('../../results/cusp/interval/{}'.format(dt)):
        os.makedirs('../../results/cusp/interval/{}'.format(dt))

    for al in [0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2]:
        for bl in [4, 4.5, 5, 5.5, 6, 6.5, 7, 7.5, 8, 8.5]:

            df_tseries = pd.read_csv('../cusp_data_interval/{}/cusp_data_{}_{}.csv'.format(dt,al,bl))
            ...
            dic_W_out.to_csv('../../results/cusp/interval/{}/cusp_W_out_{}_{}.csv'.format(dt,al,bl),header=True,index=False)