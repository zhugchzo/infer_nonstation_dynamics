...
class ReservoirComputing:
    ...
    def multi_step_predict(self, X_start, num_steps, t_series):
        """
        ...
        for t in range(num_steps):
            # 当前的输入包括x(t)和已知的t(t)，并预测x(t+1)
            current_input = np.array([last_x.item(), t_series[t].item()])  # 输入是 x(t) 和 t(t)
...
for al in [0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2]:
    for bl in [4, 4.5, 5, 5.5, 6, 6.5, 7, 7.5, 8, 8.5]:
        ...
        X_train = np.hstack([x_tseries[:train_length], t_series[:train_length]])  # 输入是(x, t)
        Y_train = x_tseries[1:train_length + 1]  # 目标是预测下一时刻的x

        # 测试数据（后test_length部分）
        X_test = np.hstack([x_tseries[train_length:-1], t_series[train_length:-1]])  # 输入是(x, t)
        ...
        # 使用多步预测方法进行预测
        predict_traj = rc.multi_step_predict(initial_input, num_steps=test_length, t_series=t_series[train_length:])
        ...
        pred_out.to_csv('../../results/cusp/competing/cusp_pred_RCt_{}_{}.csv'.format(al,bl),header = True, index=False)