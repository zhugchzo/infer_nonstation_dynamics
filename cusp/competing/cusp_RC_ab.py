...
if not os.path.exists('../../results/cusp/competing'):
    os.makedirs('../../results/cusp/competing')
...
class ReservoirComputing:
    ...
    def multi_step_predict(self, X_start, num_steps, ab_series):
        """
        进行多步预测 每一步的预测结果作为下一步的输入
        """
        predictions = []
        last_x = X_start[0]  # 初始输入x值
        
        for t in range(num_steps):
            # 当前的输入包括x(t)和已知的ab(t)，并预测x(t+1)
            current_input = np.concatenate(([last_x.item()], ab_series[t]))  # 输入是 x(t) 和 ab(t)
            prediction = self.predict(current_input)  # 预测 x(t+1)
            predictions.append(prediction[0])  # 保存预测的值
...
for al in [0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2]:
    for bl in [4, 4.5, 5, 5.5, 6, 6.5, 7, 7.5, 8, 8.5]:
        ...
        ab_series = np.array(values_tseries[:,2:])
        ...
        X_train = np.hstack([x_tseries[:train_length], ab_series[:train_length]])  # 输入是(x, t)
        Y_train = x_tseries[1:train_length + 1]  # 目标是预测下一时刻的x

        # 测试数据（后test_length部分）
        X_test = np.hstack([x_tseries[train_length:-1], ab_series[train_length:-1]])  # 输入是(x, t)
        ...
        # 使用多步预测方法进行预测
        predict_traj = rc.multi_step_predict(initial_input, num_steps=test_length, ab_series=ab_series[train_length:])

        dic_pred = {'Time':t_series[train_length + 1:,0],'traj':x_tseries[train_length + 1:,0],'pred':predict_traj[:,0]}

        pred_out = pd.DataFrame(dic_pred)
        pred_out.to_csv('../../results/cusp/competing/cusp_pred_RCab_{}_{}.csv'.format(al,bl),header = True, index=False)