...
if not os.path.exists('../../results/cusp/cusp_sin'):
    os.makedirs('../../results/cusp/cusp_sin')
...
for al in [0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2]:
    for bl in [4, 4.5, 5, 5.5, 6, 6.5, 7, 7.5, 8, 8.5]:
        ...
        df_tseries = pd.read_csv('../cusp_sin_data/cusp_sin_data_{}_{}.csv'.format(al,bl))
        ...
        for initial_theta in grid_initial_theta:
            ...
            for delta_theta in grid_delta_theta:
                ...
                for i in range(158, 472):
                    theta[i] = theta[i-1] - delta_theta

                for i in range(472, 786):
                    theta[i] = theta[i-1] + delta_theta

                for i in range(786, 1000):
                    theta[i] = theta[i-1] - delta_theta
                ...
                lambda_param = 1e-1  # Adjustable parameter, modify according to your data
                    ...
                    for row1 in range(dlin):
                        for row2 in range(row1,dlin):
                            for row3 in range(row2,dlin):
                                for row4 in range(row3,dlin):
                                    # shift by one for constant
                                    out_gen_sparse[dlin + cnt + cte] = x_gen_sparse[row1,j] * x_gen_sparse[row2,j] * x_gen_sparse[row3,j] * x_gen_sparse[row4,j]
                                    cnt += 1
                        ...
                        for row2 in range(row1,dlin):
                            for row3 in range(row2,dlin):
                                for row4 in range(row3,dlin):
                                    for row5 in range(row4,dlin):
                                        # shift by one for constant
                                        out_gen_sparse[dlin + cnt + cte] = x_gen_sparse[row1,j] * x_gen_sparse[row2,j] * x_gen_sparse[row3,j] * x_gen_sparse[row4,j] * x_gen_sparse[row5,j]
                                        cnt += 1
        ...
        polynomial = ['cte', 'x', 'p', 'x^2', 'px', 'p^2', 'x^3', 'px^2', 'p^2x', 'p^3', 'x^4', 'px^3', 'p^2x^2', 'p^3x', 'p^4','x^5', 'px^4', 'p^2x^3', 'p^3x^2', 'p^4x', 'p^5']
        ...
        dic_W_out.to_csv('../../results/cusp/cusp_sin/cusp_sin_known_W_out_{}_{}.csv'.format(al,bl),header=True,index=False)
        ...
        length = 1000
        theta = np.empty(length)
        theta[0] = best_initial_theta

        for i in range(1, 158):
            theta[i] = theta[i-1] + best_delta_theta
        ...
        for j in range(test_length):
            ...
            for row1 in range(dlin):
                for row2 in range(row1,dlin):
                    for row3 in range(row2,dlin):
                        for row4 in range(row3,dlin):
                            out_test[dlin + cnt + cte] = x_test[row1,j] * x_test[row2,j] * x_test[row3,j] * x_test[row4,j]
                            cnt += 1
            ...
            for row1 in range(dlin):
                for row2 in range(row1,dlin):
                    for row3 in range(row2,dlin):
                        for row4 in range(row3,dlin):
                            for row5 in range(row4,dlin):
                                out_test[dlin + cnt + cte] = x_test[row1,j] * x_test[row2,j] * x_test[row3,j] * x_test[row4,j] * x_test[row5,j]
                                cnt += 1                       
        ...
        dic_pred = {'Time':t_series[train_length + 1:],'traj':x_tseries[train_length + 1:,0],'pred':predict_traj}

        pred_out = pd.DataFrame(dic_pred)
        pred_out.to_csv('../../results/cusp/cusp_sin/cusp_sin_known_pred_{}_{}.csv'.format(al,bl),header = True, index=False)
        ...
        #             for row3 in range(row2,dlin):
        #                 for row4 in range(row3,dlin):
        #                     out_gen[dlin + cnt + cte] = x_gen[row1,j] * x_gen[row2,j] * x_gen[row3,j] * x_gen[row4,j]
        #                     cnt += 1
        ...
        #                 for row4 in range(row3,dlin):
        #                     for row5 in range(row4,dlin):
        #                         out_gen[dlin + cnt + cte] = x_gen[row1,j] * x_gen[row2,j] * x_gen[row3,j] * x_gen[row4,j] * x_gen[row5,j]
        #                         cnt += 1         
        ...
        # dic_gen = {'Time':t_series[1:],'traj':x_tseries[1:,0],'gen':gen_traj}

        # gen_out = pd.DataFrame(dic_gen)
        # gen_out.to_csv('../results/cusp/cusp_sin_known_gen_{}_{}.csv'.format(al,bl),header = True, index=False)