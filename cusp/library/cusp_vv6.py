...
dnonlin_6 = int(((dlin + 5)*(dlin + 4)*(dlin + 3)*(dlin + 2)*(dlin + 1)*dlin/720))
dnonlin = dnonlin_2 + dnonlin_3 + dnonlin_4 + dnonlin_5 + dnonlin_6
# total size of feature vector: linear + nonlinear
dtot = dlin + dnonlin

polynomial = ['cte', 'x', 'p', 'x^2', 'px', 'p^2', 'x^3', 'px^2', 'p^2x', 'p^3', 'x^4', 'px^3', 'p^2x^2', 'p^3x', 'p^4',
              'x^5', 'px^4', 'p^2x^3', 'p^3x^2', 'p^4x', 'p^5', 'x^6', 'px^5', 'p^2x^4', 'p^3x^3', 'p^4x^2', 'p^5x', 'p^6']

dt = 0.01

for tl in [500, 550, 600, 650, 700]:

    if not os.path.exists('../../results/cusp/library6/{}'.format(tl)):
        os.makedirs('../../results/cusp/library6/{}'.format(tl))
        ...
        for bl in [4, 4.5, 5, 5.5, 6, 6.5, 7, 7.5, 8, 8.5]:
            ...
            for initial_theta in grid_initial_theta:
                ...
                for delta_theta in grid_delta_theta:
                    ...
                    # fill in the non-linear part, order = 6
                    for row1 in range(dlin):
                        for row2 in range(row1,dlin):
                            for row3 in range(row2,dlin):
                                for row4 in range(row3,dlin):
                                    for row5 in range(row4,dlin):
                                        for row6 in range(row5,dlin):
                                            out_train[dlin + cnt + cte] = x_train[row1,:train_length] * x_train[row2,:train_length] * x_train[row3,:train_length] * x_train[row4,:train_length] * x_train[row5,:train_length] * x_train[row6,:train_length]
                                            cnt += 1
                                ...
                                for row3 in range(row2,dlin):
                                    for row4 in range(row3,dlin):
                                        for row5 in range(row4,dlin):
                                            for row6 in range(row5,dlin):
                                                # shift by one for constant
                                                out_gen_sparse[dlin + cnt + cte] = x_gen_sparse[row1,j] * x_gen_sparse[row2,j] * x_gen_sparse[row3,j] * x_gen_sparse[row4,j] * x_gen_sparse[row5,j] * x_gen_sparse[row6,j]
                                                cnt += 1
            ...
            dic_W_out.to_csv('../../results/cusp/library6/{}/cusp_W_out_{}_{}.csv'.format(tl,al,bl),header=True,index=False)
