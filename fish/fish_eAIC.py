...
if not os.path.exists('../results/fish/equations'):
    os.makedirs('../results/fish/equations')
...
for initial_theta in grid_initial_theta:
    ...
    for delta_theta in grid_delta_theta:
        ...
        theta_train = encoded_pars[:train_length+1]
        ...
        dic_sparse_node_dictionary['({},{})'.format(initial_theta,delta_theta)] = sparse_node_dictionary_list
        dic_ridge_node_dictionary['({},{})'.format(initial_theta,delta_theta)] = ridge_node_dictionary_list
...
#####################################################################################
        ...
        if np.log(ridge_MSE) >= 0:
            AIC_ridge = pinv_error*train_length*np.log(ridge_MSE) + 2*ridge_nozero
        else:
            AIC_ridge = train_length*np.log(ridge_MSE)/pinv_error + 2*ridge_nozero
...
AIC_out.to_csv('../results/fish/fish_AIC.csv', index_label='')
...
encoded_pars = (df_tseries['date_tag'].apply(lambda x: encode_date_tag(x, min=best_initial_theta, delta=best_delta_theta))).values
theta_test = encoded_pars[train_length:]
...
for j in range(test_length):
    ...
    for node in range(N):
        ...
        for jj in range(dtot + cte):
            df_f_elements[f_library[jj]] = [W_out[jj]]

        df_f_elements.to_csv('../results/fish/equations/{}_{}.csv'.format(node,col_fish[node]),header = True,index=False)
...
csv_out.to_csv('../results/fish/fish_pred.csv',header = True)