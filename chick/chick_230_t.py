...
dic_W_out.to_csv('../results/chick/chick_230_W_out_t.csv',header=True,index=False)
...
if pd_bif_time != 0:
    ...
    pred_2_out.to_csv('../results/chick/chick_230_pred_2_t.csv',header = True, index=False)
...
pred_1_out.to_csv('../results/chick/chick_230_pred_1_t.csv',header = True, index=False)
...
dic_gen = {'Time':t_series[1:1+len(gen_traj),0],'traj':x_tseries[1:1+len(gen_traj),0],'gen':gen_traj}

gen_out = pd.DataFrame(dic_gen)
gen_out.to_csv('../results/chick/chick_230_gen_t.csv',header = True, index=False)