...
df_tseries = pd.read_csv('chick_data_335.csv')
...
for initial_theta in grid_initial_theta:
    ...
    for delta_theta in grid_delta_theta:
        ...
        lambda_param = 1  # Adjustable parameter, modify according to your data
...
AIC_out.to_csv('../results/chick/chick_335_AIC.csv', index_label='')
...
dic_W_out.to_csv('../results/chick/chick_335_W_out.csv',header=True,index=False)
...
dic_pred_1 = {'Time':tseries[train_length + 1:train_length + 1 + len(predict_traj)],
              ...
              'pred':predict_traj,'theta':theta[train_length + 1:train_length + 1 + len(predict_traj),0]}

dic_pred_2 = {'Time':t_chaos[last+1:],'pred':x_chaos[last+1:]}

pred_1_out = pd.DataFrame(dic_pred_1)
pred_1_out.to_csv('../results/chick/chick_335_pred_1.csv',header = True, index=False)
pred_2_out = pd.DataFrame(dic_pred_2)
pred_2_out.to_csv('../results/chick/chick_335_pred_2.csv',header = True, index=False)

# generation

# create a place to store feature vectors for prediction
out_gen = np.ones(dtot + cte)               # full feature vector
x_gen = np.zeros((dlin,pd_bif_time))      # linear part

x_gen[-1,:] = x_theta_tseries[:pd_bif_time,-1]
...
dic_gen = {'Time':tseries[1:1+len(gen_traj)],'traj':x_tseries[1:1+len(gen_traj),0],'gen':gen_traj,'theta':theta[1:1+len(gen_traj),0]}

gen_out = pd.DataFrame(dic_gen)
gen_out.to_csv('../results/chick/chick_335_gen.csv',header = True, index=False)