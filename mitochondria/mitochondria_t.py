...
dic_W_out.to_csv('../results/mitochondria/mitochondria_W_out_t.csv',header=True,index=False)
...
dic_pred = {'Time':t_series[train_length + 1:,0],'traj':x_tseries[train_length + 1:,0],'pred':predict_traj}

pred_out = pd.DataFrame(dic_pred)
pred_out.to_csv('../results/mitochondria/mitochondria_pred_t.csv',header = True, index=False)
...
gen_out.to_csv('../results/mitochondria/mitochondria_gen_t.csv',header = True, index=False)
