...
if not os.path.exists('../results/mitochondria'):
    os.makedirs('../results/mitochondria')

df_tseries = pd.read_csv('mitochondria_data.csv')
...
AIC_out.to_csv('../results/mitochondria/mitochondria_AIC_AIC.csv', index_label='')
...
dic_W_out.to_csv('../results/mitochondria/mitochondria_W_out_AIC.csv',header=True,index=False)
...
dic_pred = {'Time':tseries[train_length + 1:],'traj':x_tseries[train_length + 1:,0],'pred':predict_traj,'theta':theta[train_length + 1:,0]}

pred_out = pd.DataFrame(dic_pred)
pred_out.to_csv('../results/mitochondria/mitochondria_pred_AIC.csv',header = True, index=False)
...
dic_gen = {'Time':tseries[1:],'traj':x_tseries[1:,0],'gen':gen_traj,'theta':theta[1:,0]}

gen_out = pd.DataFrame(dic_gen)
gen_out.to_csv('../results/mitochondria/mitochondria_gen_AIC.csv',header = True, index=False)
