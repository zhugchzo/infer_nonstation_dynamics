...
for rand_seed in range(100):
    ...
    dic_W_out.to_csv('../../results/Koscillators/robust/Koscillators_W_out_p_{}.csv'.format(rand_seed),header=True,index=False)
    ...
    for node in range(N):
        ...
        x_test[d+d_theta:,(test_length+1)*node] = np.sum(np.sin(xp_tseries[0,inter_var_index]-np.tile(xp_tseries[0,node],k)))
    ...
    pred_out.to_csv('../../results/Koscillators/robust/Koscillators_pred_p_{}.csv'.format(rand_seed),header = True, index=False)