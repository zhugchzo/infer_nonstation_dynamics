...
for dt in [0.02,0.03,0.04,0.05]:
    ...
    if not os.path.exists('../../results/Koscillators/interval/{}'.format(dt)):
        os.makedirs('../../results/Koscillators/interval/{}'.format(dt))

    for rand_seed in range(100):

        # Load the saved data
        df_tseries = pd.read_csv('../Koscillators_data_interval/{}/Koscillators_data_{}.csv'.format(dt,rand_seed))
        ...
        dic_W_out.to_csv('../../results/Koscillators/interval/{}/Koscillators_W_out_{}.csv'.format(dt,rand_seed),header=True,index=False)