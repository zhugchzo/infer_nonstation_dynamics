...
if not os.path.exists('../results/Koscillators'):
    os.makedirs('../results/Koscillators')

# Load the saved data
df_tseries = pd.read_csv('Koscillators_data.csv')
df_network = pd.read_csv('Koscillators_network.csv',header=None)
...
AIC_out = AIC_sparse_matrix

AIC_out.to_csv('../results/Koscillators/Koscillators_AIC.csv', index_label='')
...
# start = best_initial_theta * W_out[8] + W_out[3]
# end = start + best_delta_theta * W_out[8]*1000
...
dic_W_out.to_csv('../results/Koscillators/Koscillators_W_out.csv',header=True,index=False)
...
for node in range(N):
    ...
    x_test[d+d_theta:,(test_length+1)*node] = np.sum(np.sin(x_theta_tseries[0,inter_var_index]-np.tile(x_theta_tseries[0,node],k)))
...
pred_out.to_csv('../results/Koscillators/Koscillators_pred.csv',header = True, index=False)
