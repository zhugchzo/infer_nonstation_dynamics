...
for tl in [300, 350, 400, 450, 500]:
    ...
    if not os.path.exists('../../results/Koscillators/library5/{}'.format(tl)):
        os.makedirs('../../results/Koscillators/library5/{}'.format(tl))
    ...
    for rand_seed in range(100):
                    ...
                    for row2 in range(row1,dlin):
                        for row3 in range(row2,dlin):
                            for row4 in range(row3,dlin):
                                out_train[dlin + cnt + cte] = x_train[row1,:] * x_train[row2,:] * x_train[row3,:] * x_train[row4,:]
                                cnt += 1
                        ...
                        for row3 in range(row2,dlin):
                            for row4 in range(row3,dlin):
                                for row5 in range(row4,dlin):
                                    out_train[dlin + cnt + cte] = x_train[row1,:] * x_train[row2,:] * x_train[row3,:] * x_train[row4,:] * x_train[row5,:]
                                    cnt += 1            
                        ...
                        for row1 in range(dlin):
                            for row2 in range(row1,dlin):
                                for row3 in range(row2,dlin):
                                    for row4 in range(row3,dlin):
                                        # shift by one for constant
                                        out_gen_sparse[dlin + cnt + cte,node] = x_gen_sparse[row1,train_length*node+j] * x_gen_sparse[row2,train_length*node+j] * x_gen_sparse[row3,train_length*node+j] * x_gen_sparse[row4,train_length*node+j]
                                        cnt += 1
                            ...
                            for row2 in range(row1,dlin):
                                for row3 in range(row2,dlin):
                                    for row4 in range(row3,dlin):
                                        for row5 in range(row4,dlin):
                                            # shift by one for constant
                                            out_gen_sparse[dlin + cnt + cte,node] = x_gen_sparse[row1,train_length*node+j] * x_gen_sparse[row2,train_length*node+j] * x_gen_sparse[row3,train_length*node+j] * x_gen_sparse[row4,train_length*node+j] * x_gen_sparse[row5,train_length*node+j]
                                            cnt += 1
        ...
        dic_W_out.to_csv('../../results/Koscillators/library5/{}/Koscillators_W_out_{}.csv'.format(tl,rand_seed),header=True,index=False)
