...
for rand_seed in range(100):
    ...
    dic_W_out.to_csv('../../results/Koscillators/miss_cos-/Koscillators_W_out_p_{}.csv'.format(rand_seed),header=True,index=False)
    ...
    x_test[d:d+d_theta,:] = np.tile(xp_tseries[train_length:, -1], N)
    ...
    for node in range(N):
        ...
        x_test[0,(test_length+1)*node] = xp_tseries[0,node]
        x_test[d+d_theta:,(test_length+1)*node] = np.sum(np.cos(xp_tseries[0,inter_var_index]-np.tile(xp_tseries[0,node],k)))
    ...
    dic_pred = {'p':p_series[train_length + 1:,0]}
    ...
    pred_out.to_csv('../../results/Koscillators/miss_cos-/Koscillators_pred_p_{}.csv'.format(rand_seed),header = True, index=False)