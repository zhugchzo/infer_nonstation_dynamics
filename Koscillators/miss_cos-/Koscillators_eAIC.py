...
if not os.path.exists('../../results/Koscillators/miss_cos-'):
    os.makedirs('../../results/Koscillators/miss_cos-')
...
d_theta = 1
# input dimension of interaction (sum_cos(xj-xi))
dcop = 1
...
for rand_seed in range(100):
    ...
    # Load the saved data
    df_tseries = pd.read_csv('../Koscillators_dataN/Koscillators_data_{}.csv'.format(rand_seed))
    df_network = pd.read_csv('../Koscillators_dataN/Koscillators_network_{}.csv'.format(rand_seed),header=None)
        ...
        for delta_theta in grid_delta_theta:
            ...
            for node in range(N):
                ...
                for j in range(train_length):
                    ...
                    x_train[d+d_theta:,train_length*node+j] = np.sum(np.cos(node_data_theta[j][d:k*d]-np.tile(node_data_theta[j][:d],k-1)))
    ...
    #####################################################################################
            ...
            for node in range(N):
                ...
                x_gen_sparse[d+d_theta:,train_length*node] = np.sum(np.cos(x_theta_tseries[0,inter_var_index]-np.tile(x_theta_tseries[0,node],k)))
            ...
            for j in range(train_length-1):
                ...
                for node in range(N):
                    ...
                    x_gen_sparse[d+d_theta:,train_length*node+j+1] = np.sum(np.cos(inter_var-np.tile(x_gen_sparse[0,train_length*node+j+1],k)))
            ...
            inter_indexes = [i for i, item in enumerate(f_library) if 'cos' in item]
    ...
    dic_W_out.to_csv('../../results/Koscillators/miss_cos-/Koscillators_W_out_{}.csv'.format(rand_seed),header=True,index=False)
    ...
    x_test[d:d+d_theta,:] = np.tile(x_theta_tseries[train_length:, -1], N)
    ...
    for node in range(N):
        ...
        # copy over initial linear feature vector
        x_test[0,(test_length+1)*node] = x_theta_tseries[0,node]
        x_test[d+d_theta:,(test_length+1)*node] = np.sum(np.cos(x_theta_tseries[0,inter_var_index]-np.tile(x_theta_tseries[0,node],k)))
    ...
    for j in range(test_length):
        ...
        for node in range(N):
            ...
            x_test[d+d_theta:,(test_length+1)*node+j+1] = np.sum(np.cos(inter_var-np.tile(x_test[0,(test_length+1)*node+j+1],k)))

    dic_pred = {'Time':t_series[train_length + 1:]}
    ...
    pred_out.to_csv('../../results/Koscillators/miss_cos-/Koscillators_pred_{}.csv'.format(rand_seed),header = True, index=False)
